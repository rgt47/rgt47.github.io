...
Lets assume we're just finished developing a 'shiny' new shiny app, named ``myapp.R``,
that resides in the directory ``~/APPS/app3``. (where in my case I created the
symbolic link ``APPS`` via the z-shell to ``sh> ln -s ~/prj/c060/prj/launch ~/APPS``.
We want to check that the app is working as expected locally. To do that we can run it with the shell
command ``APPS/app3> Rscript -e "runApp("myapp.R",launch.browser=T)"``.
...
Once we have a prodeuction ready dockerized hsiny app available, ewe'll want to
host it on the interjet; mTho do that we;ll need the following :
1 ) a virtual server,
2) a domain name
